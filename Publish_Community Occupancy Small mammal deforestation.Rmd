...
## select dataframe rows based on trapID/season/occasion/species of capttot and assign 1 to detect/no-detect
```{r}
head(captanim12)
head(speoccup)
...
create a sp_a_r and sp_a_d array of trap x occasion x species
```{r}
#ntrap <- 150*9*2 # number of sites
#nrep <- 8 # max number of replicate surveys per trap
#nspec <- length(unique(speoccup$species)) # 14 species occur in the #data
#sp_a <- array(NA, dim = c(ntrap, nrep, nspec))
#
#(ordered.spec.name.list <- unique(speoccup$species)) # ID-order list
```

populate array with spespread data
```{r}
#for(i in 1:nspec){
#  
#  sp_a[,,i] <- as.matrix(spespread_tot[spespread_tot$species==ordered.s#pec.name.list[i],8:15],ntrap)
#}
#dimnames(sp_a) <- list(trapid=unique(spespread_tot$TrapID),occasion=as.#character(1:8), sps=ordered.spec.name.list)

```

create the obs.occ for comparing:
```{r}
#table(nsurveys <- apply(sp_a[,,1], 1, function(x) sum(!is.na(x))))
#(NAsites <- which(nsurveys == 0) )
#
#tmp <- apply(sp_a, c(1,3), max, na.rm = TRUE)
#sort(obs.occ <- apply(tmp, 2, sum, na.rm = TRUE))
```


## format data specifically for this analysis
```{r}
